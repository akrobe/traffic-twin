...
void Predictor::init_opencl_if_possible() {
  if (!cfg_.prefer_opencl) return;
  cl_uint np=0; if (clGetPlatformIDs(0,nullptr,&np)!=CL_SUCCESS || np==0) return;
  ...
  for (auto p: plats) {
    ...
    std::vector<cl_device_id> devs(nd); clGetDeviceIDs(p, CL_DEVICE_TYPE_GPU, nd, devs.data(), nullptr);
    cl_ = new ClCtx();
    cl_->platform=p; cl_->device=devs[0];
    cl_int err=0;
    ...
    if (clBuildProgram(cl_->prog, 1, &cl_->device, "", nullptr, nullptr)!=CL_SUCCESS) { delete cl_; cl_=nullptr; continue; }
...
void Predictor::cpu_predict(const std::vector<Features>& feats, std::vector<Prediction>& out) {
  ...
  for (int i=0;i<(int)feats.size();++i) {
    float z=bias;
    for (int j=0;j<6;++j) z += feats[i].f[j]*W[j];
...
void Predictor::predict_batch(const std::vector<Features>& feats, std::vector<Prediction>& out) {
  if (!has_cl_ || feats.empty()) { cpu_predict(feats,out); return; }
  // OpenCL path
  ...
  for (int i=0;i<B;++i) for (int j=0;j<F;++j) X[i*F+j]=feats[i].f[j];

  if (cl_->B_cap < B) {
    if (cl_->dX) { clReleaseMemObject(cl_->dX); clReleaseMemObject(cl_->dW); clReleaseMemObject(cl_->dO); }
  ...
  out.resize(B);
  for (int i=0;i<B;++i) out[i] = Prediction{feats[i].ts_ms, feats[i].junction, O[i]};
}