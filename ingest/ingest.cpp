...
void Ingestor::generate(uint32_t tick_id, std::vector<SensorSample>& out) {
  ...
  for (uint32_t j=0; j<cfg_.junctions; ++j) {
    for (uint32_t l=0; l<cfg_.lanes_per; ++l) {
      ...
      s.arrivals = (uint16_t) std::max(0, int((8 + b) * peak + noise) * 10);
      s.q_len = (uint16_t) std::max(0, int((b + (peak>1.0f?5:1)) + std::max(0.f, noise)));
      s.avg_speed = (uint16_t) std::max(1, 50 - s.q_len) * 10;
      out.push_back(s);
    }
  }
}